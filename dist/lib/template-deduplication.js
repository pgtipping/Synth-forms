...
export class TemplateDeduplicate {
    ...
    isExcluded(filePath) {
        return config.excludePatterns.some(pattern => filePath.toLowerCase().includes(pattern.toLowerCase()));
    }
    ...
    async cleanupDuplicates(duplicates) {
        ...
        for (const group of duplicates) {
            ...
            for (const file of toMove) {
                const backupPath = path.join(cleanupDir, path.basename(file.path));
    ...
    async generateReport(duplicates) {
