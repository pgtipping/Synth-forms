...
export class Validator {
  static validateTemplate(data: Partial<Template>): boolean {
    const errors: string[] = [];
    ...
    if (data.formDefinition) {
      if (!Array.isArray(data.formDefinition.fields)) {
        ...
        data.formDefinition.fields.forEach((field, index) => {
          ...
          errors.push(
            ...fieldErrors.map((err) => `Field ${index + 1}: ${err}`)
          );
        });
      }
    }
  ...
  static validateFormFieldDefinition(field: FormField): string[] {
    const errors: string[] = [];
    ...
    if (field.dependencies) {
      field.dependencies.forEach((dep, index) => {
        ...
        if (!dep.condition?.operator)
          errors.push(
            `Dependency ${index + 1}: Condition operator is required`
          );
        if (!dep.action)
  ...
  static validateCustomization(data: Partial<Customization>): boolean {
    const errors: string[] = [];
  ...
  static validateFormResponse(data: Partial<FormResponse>): boolean {
    const errors: string[] = [];
  ...
  static processTemplateData(data: Partial<Template>): Partial<Template> {
