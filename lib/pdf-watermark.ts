...
async function detectTextWatermark(page: any, pageNumber: number): Promise<WatermarkDetectionResult | null> {
  try {
    ...
    for (const item of items) {
      ...
      const hasExactMatch = config.watermark.textKeywords.some(keyword => 
        textLower.includes(keyword.toLowerCase())
      );
      
      const hasPatternMatch = config.watermark.patterns && (
        config.watermark.patterns.fullPattern.test(text) ||
...
async function detectImageWatermark(page: any, pageNumber: number): Promise<WatermarkDetectionResult | null> {
  try {
    ...
    for (const op of operatorList.fnArray) {
      if (op === pdfjs.OPS.paintXObject || op === pdfjs.OPS.paintImageXObject) {
        ...
        const hasTransparency = imgData.data.some((value: number, index: number) => index % 4 === 3 && value < 255);
...
export async function detectPDFWatermark(filePath: string): Promise<WatermarkDetectionResult> {
  try {
